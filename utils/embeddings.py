...
def get_w2v_embeddings_from_path(model_path, padding=False):
    """
        Arguments:
            model_path {String} -- gensim word2vec model file path
...
def get_w2v_embeddings_from_model(word2vec: gensim.models.word2vec, padding: object = False) -> object:
    """
        Arguments:
            model {String} -- gensim word2vec model
            padding {Boolean} -- Whether to expand a 0 vector for padding
        Returns:
            embedding {Tensor, shape [n_vocabulary, embedding length]} -- word embedding vectors
            vector_length {Int} -- embedding length
